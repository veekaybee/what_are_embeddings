...
[
  ...
  \def\d{.9}
  \coordinate (u) at (\d,0);
  \draw [myArrows] ($(right-out)-(u)$) node [anchor=east] {Embeddings};
  \draw [myArrows] ($(left-out)-(u)$) node [anchor=east] {Input\\text} -- ++(u) ;
...
The most critical part of any machine learning system that works in production is inference time --- how quickly does it take to query the model asset and return a result to an end-user. For use-cases like search and loading a feed of content, the experience needs to be instantaenous or the user experience will degrade. 

We can't foreget that we're operating not only in a theoretical, but practical engineering environment. 
