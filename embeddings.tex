...
\section*{Colophon }
This paper is typeset with  \LaTeX. The cover art is Kandinsky's "Composition, VII" , 1913. \href{https://vickiboykis.com/2023/02/26/what-should-you-use-chatgpt-for/}{ChatGPT was used} to generate some of the figures. 

\section*{Code}
The latest version of the paper and code examples are available here. \href{http://vickiboykis.com/what_are_embeddings/}{here}. The code examples is available \href{https://github.com/veekaybee/what_are_embeddings}{here}. PRs welcome.

\section*{About the Author}
Vicki Boykis is a machine learning engineer. Her website is \href{https://www.vickiboykis.com}{vickiboykis.com} and Viberary, her semantic search side project is \href{https://www.viberary.pizza}{viberary.pizza}. Her email is \href{mailto:vickiboykis@gmail.com}{vickiboykis@gmail.com}.
...
We've highlighted this quote because we'll be working with this sentence a fair bit as our input example throughout this text. 
...
\subsection{The Task of Recommendations}
...
In three dimensions, with three features icnluding the bird id, we would have a matrix
\begin{equation}
\begin{bmatrix}
0 & 0 & 12\\
0& 2& 0 \\
...
\end{minted}
\caption{Implementation of cosine similarity in scikit}
\end{figure}


\subsubsection{SVD and PCA}

Several other related early approaches were used in lieu TF-IDF for creating compact representations of items: \textbf{principal components analysis}(PCA) and \textbf{singular value decomposition}(SVD).  
...
The goal of the SVM is to find the optimal hyperplane such that the distance between new projections of objecgts (words in our case) into the space maximizes the distance between the plane and the elements so there's less chance of mis-classifying them. 
...
Let's start from our input data.  In this case, our corpus are all of the flit we've collected. We first need to process them as input into our model. 
...
At the end of each epoch, or pass through the model, we pass the weights, or \textbf{backpropogate} them, back to the linear layer,and then again, update the embeddings table with the distance between the words, based on the probability. The probability is calculated through a \textbf{softmax} function, which converts a vector of real numbers into a probability distribution - that is, each number in the fector, i.e. the value of the probability of each words, is in the interval between 0 and 1 and all of the word numbers add up to one.   The distance, as backpropogated to the embeddings table, should converge or shrink depending on the model understanding how close specific words are. 
...
Once we've completed our iteration through our training set, we have learned a model that retrieves both the probability of a given word being the correct word, and the entire embedding space for our vocabulary\footnote{For an illustrated walkthrough of Word2Vec, check  \href{https://jalammar.github.io/illustrated-word2vec/}{here}}. 

\section{Modern Embeddings Architectures}

Word2Vec became one of the first neural network approaches to use the concept of embedding to create a vocabulary, but neural networks as a whole took off. Several key factors led to the rise and success of neural network architectures. First, Geoffrey Hinton, a pioneer in the field of deep learning, co-published a paper on the method of backpropogation in the 1980s\citep{rumelhart1986learning}, the process of training many variations of neural networks. During the backpropogation pass over a network, the algorithm computes the gradient of the loss function with respect to the weights of the neural network using the \textbf{chain rule}. Previously, each of these calculations had to be done individually and this process was computationally ineffective. 
...
The final two layers of the Transformer model are a liner layer, which projects the float vector of embeddings plus positional encodings into a word by turning the score at each vector position into words with the help of a softmax layer, which turns the score into probabilities. The highest score is the output of what the word should be, plus its index in the embedding matrix. 

Initially,  we have our input data, the matrix operations of the encoder that embeds the text in the space of continuous numerical representations, and the decoder takes those numerical representations, one element at a time and generates an output sequence.
...
We then get the predicted word, just like in Word2Vec.

Other than learning context well, Transfomers are also able to parallelize their training by passing multiple tokens to the encoder at the same time, computing states for all positions in the input text. 

\subsection{BERT}

After the success of "Attention is All you Need", a variety of transformer architectures arose. \textbf{BERT} 
stands for Bi-Directional Encoder and was released 2018\citep{devlin2018bert}, based of a paper written by Google as a way to solve common natural language tasks like sentiment analysis, question-answering, and text summarization. BERT is a transformer model, also developed at Google, based on the attention mechanism, but its architecture is such that it only includes the encoder piece. Its most prominent usage is in Google Search, where it's the algorithm powering surfacing relevant search results. In the blog post they released on including BERT in search ranking in 2019, Google specifically discussed adding context to queries as a replacement for keyword-based methods as a reason they did this.\footnote{\href{https://blog.google/products/search/search-language-understanding-bert/}{BERT search announcement}}
...
\begin{tikzpicture}
[
  node distance=5mm, % updated node distance value
  every node/.style={align=center},
]
  ...
  \def\d{.9}
  \coordinate (u) at (\d,0);
  \draw [myArrows] (right-out) -- ++(u) node [anchor=west] {Embeddings};
  \draw [myArrows] ($(left-out)-(u)$) node [anchor=east] {Input\\text} -- ++(u) ;
...
The other major difference between BERT and Word2Vec is that Word2Vec generates one embedding per token, whereas BERT learns multiple contexts for each given word. 

\subsection{Modern Embedding Architectures in Recommendations and Beyond}

With the advent of Transformer models, and more importantly, BERT, generating representations of large, multimodal objects for use in all sorts of machine learning tasks suddenly became much easier, the representations became more accurate, and if the company had GPUs available, computations could now be computed with speed-up in parallel through multiple layers. 

Companies began using deep learning for all sorts of NLP-related and recommendation-adjacent tasks. For example, YouTube was one of the first companies to release a paper on embeddings used in the context of a production recommender system with "Deep Neural Networks for YouTube Recommendations."\citep{covington2016deep}. In it, they discuss the need for scale given the large size of YouTube's user base

Now that we've seen a couple examples at 

Now that we've gotten tot he context of using BERT, how would we incorporate it in our recommendations. Given that we have a set of pre-trained embeddings, how might we go about recommending flits? 

We can now base recommendations on content instad of inferring the context from usage patterns as we do in collaborative filtering. We can start by genrating embeddings for all our flits in the dataset. 


\subsection{Fine-Tuning Embeddings}

Variations of BERT have been used to generate embeddings to use as downstream input into many recommender and information retrieval systems. 
...
Many companies are working with embeddings today. 
...
The explosion of embeddings-based models and the ease with which embeddings can be used as flexible inputs at many steps of the modeling and inference process has resulted in the need to operationalize them. Generally, when we talk about operationalizing machine learning, we mean that we need to make our data inputs and outputs systematic. 

In ML systems, we need several key pieces to operationalize any given system: 

\begin{formal}
\begin{itemize}
  \item Input data and a way to track it
  \item A way to generate that data
  \item A way to retrieve the model
\end{itemize}
\end{formal}

\textbf{Embeddings Generation}
...
\textbf{Embeddings Storage and Retrieval }
...
Initially, they might be used in that model, that predicts flit similar to the ones we wanted to look at.  
