...
\begin{figure}[H]
\centering{\includegraphics[width=\textwidth]{figures/youtube.png}}
...
In fact, there are so many uses for embeddings at twitter that the ML platform engineeering team created a centralized repository and store for them

Finally, at Pinterest, researchers and engineers developed PinnerSage, a two-stage embedding system that finds semantically similar Pins\citep{pal2020pinnersage}. 

There are multiple examples of deep learning embeddings being used in recommendation tasks: a wide and deep model for App Store recommendations at Google Play\citep{zhang2019deep}, dual embeddings for product complementary content recommendations at Overstock\citep{kvernadze2022two}, personalization of search results at Airbnb via real-time ranking\citep{grbovic2018real},  
...
Now that we have a single model that generates embeddings  but now we have a problem: we have to operationalize them. Embeddings are great representations of our know 
...
Pretrained embeddings are based on enormous internet corpuses.  There are a lot of pretrained word embeddings available: Word2Vec via GenSim, GLoVe, and FastText. BERT embeddings are also available.  You can also query OpenAI embeddings, although doing so comes at a fairly steep cost. 
