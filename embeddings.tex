...
\section{Modern Embedding Architectures in Recommendations and Beyond}
...
Now that we understand what embeddings are, what can we do with them from a machine learning perspective? 
...
There are numerous cases of companies training models for their own domain and their own use. For example, 

\end{itemize}
\end{formal}


Given the flexibility of embeddings as data structures that can be used anywhere, several key uses came from them:
...
Given these endless variations and use-cases, companies began using deep learning for all sorts of NLP-related and recommendation-adjacent tasks. For example, YouTube was one of the first companies to release a paper on embeddings used in the context of a production recommender system with "Deep Neural Networks for YouTube Recommendations."\citep{covington2016deep}. In it, they discuss the need for scale given the large size of YouTube's user base

Twitter, our model for flitter, of course, 

Pinterest


Embeddings at Flitter - Now that we've gotten tot he context of using BERT, how would we incorporate it in our recommendations. Given that we have a set of pre-trained embeddings, how might we go about recommending flits?  We can now base recommendations on content instad of inferring the context from usage patterns as we do in collaborative filtering. We can start by genrating embeddings for all our flits in the dataset. 

We can use embeddings either as inputs into the data model, or as 


\subsection{Considerations for Embeddings in Production}
