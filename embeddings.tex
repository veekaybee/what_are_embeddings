...
\section*{Code}
The latest version of the paper and code examples are available here. \href{https://github.com/veekaybee/embeddings2}{here}. The code examples is available \href{https://github.com/veekaybee/embeddings_code}{here}. PRs welcome. \href{https://github.com/veekaybee/embeddings}{the project repo here}.
...
After the success of Attention is All you Need, a variety of transformer architectures arose. \textbf{BERT} is a transformer model, also developed at Google, based on the attention mechanism, but its architecture is such that it only includes the encoder piece. 

The canonical model used these days for the same things that Word2Vec and LSA/LDA were used for previously is BERT. BERT stands for Bi-Directional Encoder and was released 2018\citep{devlin2018bert}, based of a paper written by Google as a way to solve common natural language tasks like sentiment analysis, question-answering, and text summarization. Perhaps its most prominent usage is in Google Search, where it's the algorithm powering surfacing relevant search results. In the blog post they released on including BERT in search ranking in 2019, Google specifically discussed adding context to queries as a replacement for keyword-based methods as a reason they did this.\footnote{\href{https://blog.google/products/search/search-language-understanding-bert/}{BERT search announcement}}

BERT is a model based on the mechanism behind transformers, but it takes it a step further. When we created our representations with Word2Vec, we only looked at sliding windows moving forward. The B in Bert is for bi-directional, which means it pays attention to words in both ways through scaled dot-product attention. 
...
\subsection{GPT and Beyond}

We've now reached the forefront of what's possible with embeddings in this paper. With the rise of generative methods and methods based on Reinforcement Learning with Human Feedback like OpenAI's ChatGPT and Microsoft's Sydney , anything written in this paper would already be impossibly out of date by the time it was published. 

Let's spend a bit of time talking specifically about the GPT series of models and how large language models work in context. 

But these methods do not exist in isolation. They are built on years 



\section{Embeddings in Production}

Now that we have embeddings, generated in a number of different ways, we can use them for downstream machine learning tasks. In the context of recommendations, we can build recommender models that will use these embeddings as features and perform any given number of tasks, from predicting whether a user will like a flit, to offering similar flit to users to recommend. 

compare words, sentences, and other groups of speech elements programmatically and mathematically through a variety of distance measures, most commonly cosine similarity, to look for similarity between them. 

Word2Vec gives us a handy way to perform recommendations: 

Let’s say we, in this case, have a matrix of Flutter users and flit that they’ve rated. Just as always, we create a matrix. Except, instead of users and words as our feature generation step, we put users on the X-axis and image names on the Y-axis. It feels like we’re again creating a feature matrix. But in the case of collaborative filtering, we perform matrix factorization. 
...
Data is the most critical part of the model, and Transformers removed the necessity to train our own, resulting in a wide variety of available. 
...
In order to perform computations on embeddings, you will need a system that is amenable to performing lookups on vectors, and to store vectors and their related components. 
